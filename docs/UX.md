...
- 사이드바 본문은 접이식 프로젝트 트리다. 각 프로젝트 행 아래에 그 프로젝트의 세션들이 표시된다
- **프로젝트 `>`/`v` 버튼**: 해당 프로젝트의 세션 목록 접기/펼치기
- **프로젝트 좌클릭**: 해당 프로젝트를 활성화하고 그 프로젝트의 첫 세션 그룹을 표시. 접혀 있으면 자동으로 펼침
- **세션 좌클릭**: 그 세션의 그룹으로 화면 전환. 그룹 안에 여러 leaf가 있으면 모두 보이고, 클릭한 세션이 그 leaf의 활성 탭이 됨
- **세션 더블클릭**: 세션 별명 변경 팝업 열기
...
- **드래그**: 펼쳐진 모든 프로젝트의 세션을 패널 위로 끌어 같은 드롭 존 시스템 사용. 다른 프로젝트 세션도 현재 작업 화면의 탭/분할로 배치 가능
