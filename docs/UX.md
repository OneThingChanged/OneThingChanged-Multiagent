...
- 위쪽은 프로젝트 목록, 아래쪽은 선택된 프로젝트의 세션 목록이다
- **프로젝트 좌클릭**: 해당 프로젝트를 활성화하고 그 프로젝트의 첫 세션 그룹을 표시
- **세션 좌클릭**: 그 세션의 그룹으로 화면 전환. 그룹 안에 여러 leaf가 있으면 모두 보이고, 클릭한 세션이 그 leaf의 활성 탭이 됨
- **MD 버튼**: 오른쪽 Docs 패널 열기/닫기. 현재 활성 프로젝트 폴더의 Markdown 파일을 목록/미리보기로 표시
- **설정 버튼**: 전역 설정 팝업 열기. 앱 테마를 바꾸면 사이드바, 탭, 모달, 컨텍스트 메뉴, 터미널, Docs 뷰어에 같이 적용됨
- **+ 버튼**: 활성 프로젝트가 있으면 새 세션 생성, 없으면 새 프로젝트 생성
- **Projects 제목 옆 + 버튼**: 새 프로젝트 생성
- **세션 우클릭**: 컨텍스트 메뉴
  - 전환 (현재 그룹으로 이동)
  - 탭으로 추가 — 활성 패널에 새 탭으로 합침
  - 오른쪽 분할 — 활성 패널 옆에 새 패널 (수평)
  - 아래로 분할 — 활성 패널 아래에 새 패널 (수직)
  - 세션 별명 변경 — 사이드바/탭/Docs subtitle에 보이는 이름 수정
  - 현재 세션으로 그룹 고정 — 그룹 안에서 저장된 Codex/Claude 세션 ID를 고정
  - 그룹 세션 고정 해제 — 고정된 세션 ID를 제거하고 다시 최신 세션 resume 방식으로 복귀
- **× 버튼**: 세션 영구 삭제 (PTY kill + sidebar/layout/storage에서 모두 제거)
- **드래그**: 사이드바 아이템을 패널 위로 끌어 같은 드롭 존 시스템 사용

### 사이드바 시각

- 활성 프로젝트: 프로젝트 목록에서 왼쪽 파란 막대와 강조 배경
- 활성 그룹 멤버: 옅은 파란 배경 + 왼쪽 막대 (멤버 ≥ 2일 때 활성이면 파란 막대, 비활성이면 회색)
- 활성 leaf의 활성 탭에 해당하는 세션: 진한 파란 배경
- 세션이 고정된 그룹 멤버: 이름 옆에 `PIN` 배지 표시
- 그룹 사이: 가는 회색 구분선
- 세션 항목은 2줄: 위 = 상태점 / 도구 아이콘 / 별명 / dangerous `!` / 닫기 `x`, 아래 = 세션 ID 앞 8자리 또는 `new session`
...
- 현재 활성 프로젝트 폴더에서 `*.md`, `*.markdown` 파일을 재귀적으로 찾음
...
- 터미널 출력의 Markdown 경로(`docs/README.md`, `Docs/Foo.md:42`, `K:\...\README.md` 등)는 링크처럼 클릭 가능. 클릭 시 해당 세션의 프로젝트 폴더 안 파일인지 확인한 뒤 Docs 패널에서 열림
